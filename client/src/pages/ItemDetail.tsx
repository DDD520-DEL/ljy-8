...
import { itemApi, orderApi, reviewApi, queueApi, favoriteApi, favoriteFolderApi } from '../api';
import { useAuthStore } from '../store/authStore';
import type { ItemWithOwner, ReviewWithUser, QueueEntryWithDetails, FavoriteFolder } from '../types';
...
function ItemDetail() {
  ...
  const loadFavoriteStatus = async () => {
    const res = await favoriteFolderApi.checkFavoriteStatus(id!, 'item');
    if (res.success) {
  ...
  const handleToggleFolderFavorite = async (folderId: string) => {
    ...
    if (isInFolder) {
      ...
      if (folderItems.success) {
        const favoriteItem = (folderItems.data || []).find((f: any) => f.targetId === id && f.targetType === 'item');
        if (favoriteItem) {
      ...
      const res = await favoriteFolderApi.addToFolder(folderId, { targetId: id!, targetType: 'item' });
      if (res.success) {
            ...
            <div className="folder-picker-content">
              {folders.length === 0 ? (
                <div className="empty-folders">
                  <p>还没有收藏夹</p>
                  <p className="text-muted">创建一个收藏夹来收藏你喜欢的物品吧</p>
                </div>
              ) : (
